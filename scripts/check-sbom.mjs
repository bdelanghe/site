...
// in-toto/SLSA statement. Same process.exit(1) contract as the css-token-purity
// gate: a violation refuses the build rather than shipping an incomplete bill.
//
// It enforces three things over dist/sbom.spdx.json (+ dist/attestation.intoto.json
// + flake.lock), all decidable / order-free:
...
//   2. Pinned-set ⊆ SBOM — every Nix flake input (nixpkgs + the brand submodule)
//      pinned in flake.lock appears as an SPDX package at the SAME rev, and any
//      package-reference (pkg:…) the attestation enumerates as a resolvedDependency
//      (i.e. the brand) appears in the SBOM at the SAME rev. (File-path materials —
//      data/*.json, brand/*.css — are content inputs, not redistributable packages,
//      so they are intentionally out of SBOM scope.)
...
const attPath = join(root, "dist", "attestation.intoto.json");
if (!(await exists(sbomPath))) { console.error("✗ check:sbom: dist/sbom.spdx.json missing — run node scripts/gen-sbom.mjs first"); process.exit(1); }
...
// 2b. every package-reference material in the attestation is in the SBOM (the brand)
if (await exists(attPath)) {
  ...
  console.warn("… check:sbom: dist/attestation.intoto.json not present — skipping attestation cross-check (run the full build to enforce it)");
}
...
console.log(`✓ check:sbom: SPDX-2.3 well-formed · ${sbom.packages.length} packages · pinned set (${flakeRevs.size} flake inputs) ⊆ SBOM ⊆ pinned set — SBOM ↔ attestation reconciled.`);
